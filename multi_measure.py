    ...
    for article_info in annotations["data"]:
        for paragraph_info in article_info["paragraphs"]:
            for qa_pair in paragraph_info["qas"]:
                query_id = qa_pair["id"]
                t = qa_pair["answer_type"]
                candidate_answers = [answer["text"] for answer in qa_pair["answers"]]
                answers_dict[query_id] = [candidate_answers, t]
    return answers_dict
...
def evaluate_json(
    ...
    for query_id, candidate_answers in annotated_answers.items():
        candidate_answers, t = candidate_answers
        if 'order' in t or 'table' in t:
            continue
        max_em_score = 0.0
        max_f1_score = 0.0
    ...
    print("##################################")
    print("Ordered List")
    instance_exact_match, instance_f1 = [], []
    a = 0
    for query_id, candidate_answers in annotated_answers.items():
        candidate_answers, t = candidate_answers
        if 'order' not in t:
    ...
    for query_id, candidate_answers in annotated_answers.items():
        ...
        if query_id in predicted_answers:
            predicted = predicted_answers[query_id]
            gold_answer = tuple(candidate_answers)
            mean_spans.append(len(gold_answer))
            mean_pred_spans.append(len(predicted))
            if len(gold_answer) > max_len:
                max_len = len(gold_answer)
            # if len(gold_answer) > 5:
            #     print("??")
            # predicted = predicted[:-1] if len(predicted) > 8 else predicted
            em_score, f1_score = drop.get_metrics(predicted, gold_answer)
            if gold_answer[0].strip() != "":
                max_em_score = max(max_em_score, em_score)
                max_f1_score = max(max_f1_score, f1_score)
        ...
        instance_exact_match.append(max_em_score)
        instance_f1.append(max_f1_score)
    print(np.mean(mean_spans))
    print(np.mean(mean_pred_spans))
    print(max_len)
    global_em = np.mean(instance_exact_match)
    global_f1 = np.mean(instance_f1)
    print("Exact-match accuracy {0:.2f}".format(global_em * 100))
...
def evaluate_prediction_file(
    prediction_path: json, gold_path: str, output_path: Optional[str] = None
) -> Tuple[float, float]:
    """
    ...
    """
    predicted_answers = prediction_path
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        default=os.path.join(DATASET_PATH, 'train',"refine_test.json"),
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        default="./results/predictions_6500_thres_7.json",
    ...
    predictions = json.load(open(args.prediction_path, 'r', encoding='utf8'))
    
    
    evaluate_prediction_file(predictions, args.gold_path, args.output_path)
